...
```{r}
model1<-lm(dbrwt~tobacco+ormoth+mrace3+dmage+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10, test_data)
model1
```
...
```{r}
#without ormoth
model1b<-lm(dbrwt~tobacco+mrace3+dmage+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10, test_data)
#without mrace3
model2b<-lm(dbrwt~tobacco+ormoth+dmage+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10, test_data)
#without dmage
model3b<-lm(dbrwt~tobacco+ormoth+mrace3+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10, test_data)
#without dmeduc
model4b<-lm(dbrwt~tobacco+ormoth+mrace3+dmage+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10, test_data)
...
```{r}
#adding in herpes, cntocpop, adequacy, and monpre
model3d<-lm(dbrwt~tobacco+ormoth+mrace3+dmage+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10+herpes+cntocpop+adequacy+monpre, my_data)
...
nonsmokers_model<-lm(tobacco~., nonsmokers)
smokers_model<-lm(tobacco~., smokers)
...
ATE <- mean(smokers_predict) - mean(nonsmokers_predict)

#Next get ATT
ATT <- mean(smokers$dbrwt - smokers_predict) - mean(nonsmokers$dbrwt - nonsmokers_predict)

#Do I need p-value?

#oaxaca package given team feedback on what this should be like
oaxaca_results <- oaxaca(dbrwt ~ ormoth+mrace3+dmage+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10 | tobacco, test_data)
#get ATE and  ATT - not sure how to do that
q3eATE<-oaxaca_results$reg$reg.pooled.2$tobacco
```
...
plot_data <- data.frame(
  ...
  treatment = factor(rep(c("Smokers", "Nonsmokers"), each = length(pscore)))
)

ggplot(plot_data, aes(x = pscores, fill = treatment)) +
...
q4dATE <- mean(match_data$birthweight[match_data$smoking == 1] - match_data$birthweight[match_data$smoking == 0])
q4dATT <- mean(match_data$birthweight[match_data$smoking == 1])
```
...
weight_data<-weightit(tobacco~ormoth+mrace3+dmage+dmeduc+nlbnl+dlivord+totord9+dfage+orfath+dfeduc+pre4000+preterm+nprevist+isllb10, test_data, method="ps")

#reweighting
weights<-weights(weight_data)

#get overall mean
q4eOverallMean <- weighted.mean(test_data$dbrwt, w = weights)
q4eATT<-weighted.mean(test_data$dbrwt[test_data$tobacco == 1], w = weights[test_data$tobacco == 1])
q4eATE<-q4eOverallMean-q4eATT
```
...
```
b. Assuming constant effects,7 estimate the causal effect using post-double selection LASSO. Use flexible specifications with some polynomials and interaction terms. (Defining these nonlinear terms — “feature engineering” — for the use in LASSO may require some thinking.) How many covariates did you start with? How many were selected by LASSO in the outcome regression? How many in the propensity score regression? How many overlapped? In this application, would you get a very different answer if you didn’t include the covariates chosen in the propensity score regression?
```{r}
#load glmnet and hdm
library(hdm)
library(glmnet)

#Perform feature engineering (dplyr mutate etc etc)
lasso_data<-test_data%>%mutate()

#Split into outcome and exposure
q5y<-lasso_data%>%select(dbrwt)
q5x<-lasso_data%>%select(-dbrwt)

#use hdm's double selection command
select_vars<-double_selection(q5x, q5y)
selectX<-which(select_vars)
selectX<-q5x[, selectX]

#run glmnet to get best value for lambda
lasso_model<-cv.glmnet(selectX, q5y, alpha=1)
best_lambda<-lasso_model$lambda.min

#fit model with best lambda
best_lasso<-glmnet(selectX, q5y, alpha=1, lambda=best_lambda)
