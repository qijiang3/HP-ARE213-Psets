...
q4dATE <- mean(match_treated$birthweight) - mean(match_untreated$birthweight)
q4dATT <- mean(match_treated$birthweight-match_untreated$birthweight)
```
...
weight_data<-weightit(tobacco~ormoth + mrace3 + dmage + dmeduc + dmar + adequacy + monpre + nprevist + isllb10 + csex + dplural + anemia + cardiac + lung + diabetes + herpes + chyper + phyper, test_data, method="ps")
...
#get ATE
q4eATE <- weighted.mean(weighted_data$dbrwt[weighted_data$tobacco == 1], w = weights[weighted_data$tobacco == 1])- weighted.mean(weighted_data$dbrwt[weighted_data$tobacco == 0], w = weights[weighted_data$tobacco == 0])
...
q4eATT <- mean(weighted_dbrwt_smokers - weighted_dbrwt_nonsmokers)
```
...
for (covariate in colnames(q5x)) {
  if (covariate != "tobacco") {
    interaction_term <- q5x$tobacco * q5x[[covariate]]
    q5x[[paste("tobacco_", covariate, sep = "")]] <- interaction_term
  }
}
#Add a polynomial for age
...
#convert y into a numeric vs a one column dataframe
q5y <- q5y[[1]]

#run glmnet to get best value for lambda
lasso_model<-cv.glmnet(q5x, q5y, alpha=1)
best_lambda<-lasso_model$lambda.min

#fit model with best lambda
best_lasso_model<-glmnet(q5x, q5y, alpha=1, lambda=best_lambda)
lasso_coef <- coef(best_lasso_model, s=best_lambda)

# Filter out nonzero coefficients (AA: added debug for exluding the intercept because kept getting an issue with subscript out of bounds)
nonzero_coef <- which(lasso_coef[-1, ] != 0)  
select_X <- q5x[, nonzero_coef]

#then get linear model? Is this right?
ds_model<-lm(q5y~select_X)
